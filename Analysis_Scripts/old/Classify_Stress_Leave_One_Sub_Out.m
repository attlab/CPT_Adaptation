...
%     end
    ...
    for permuteLabels=1:2
        ...
        % 1-7 currently all measures exect HFHRV, 8 is ALL measures left in
        for iLeaveOutMeasure=1:8
            
            iLeaveOutMeasure
            
            if iLeaveOutMeasure~=8
                keepMeasure =1:7;
            ...
            for iter=1:nIter
                ...
                % loop through subjects, leaving one absent on each iter
                ...
                for iSub=0:27
                    ...
                    % determine left out trial indices
                    testMemberIdx = (iSub*10)+1:(iSub*10)+10;
                    
                    % which trials to keep
                    allTrialsIdx(testMemberIdx) = [];
                    trainMemberIdx = allTrialsIdx;
                    
                    % set training and testing datasets
                    testData = allData(testMemberIdx,:);
                    trainData = allData(trainMemberIdx,:);
                    
                    % set condition labels
                    testMember = condLabels(testMemberIdx);
                    trainMember = condLabels(trainMemberIdx);
                    ...
                    % get accuracy scores for each sub
                    nTrials=10;
                    accData(iSub+1,iLeaveOutMeasure,iter) = correctTrials/nTrials;
...
save([destDir '/' 'Classification_Results_Leave_Sub_Out.mat'],'classResults')
...
%                     nTrials = 10;
%                     
%                     % loop through this subject's 10 test trials
%                     correctTrials=0;
%                     for iTestTrial=1:nTrials
%                     
%                             % isolate specific trial for testing
%                             testData = testSubData(iTestTrial,:);
%                             testMember = testSubCondLabels(iTestTrial);
%                             
%                             p=repmat(1/2,1,2);
%                             
%                             Labels=classify(testData,trainData,trainMember,'lin',p);
