...
for iFreq=freqIdx
    ...
    h=figure;
    set(gcf, 'Units', 'Normalized', 'OuterPosition',[0 0.0049 0.7090 0.9679]); % replace 1 with .8 to get back to normal
...
%                   cbar
    ...
    for iTimes=2:4
        for iPlot=1:3
            ...
            if      iPlot==1; theseData = condVec(:,iTimes); plotPos = [46.2,49.2,52.2]; thisTitle = 'Cond';
            elseif  iPlot==2; theseData = trialVec(:,iTimes); plotPos = [47,50,53]; thisTitle = 'Trial';
            elseif  iPlot==3; theseData = intVec(:,iTimes); plotPos = [47.8,50.8,53.8]; thisTitle = 'Int';
            end
            ...
            t=subplot(6,9,plotPos(iTimes-1));
    ...
    if analysisType==1
        ...
        saveas(h,[destDir '/' 'EEG_ERSP_1-30Hz_Topos_Brain70_Dip15_TimeGraded' thisFreqName '.eps'],'epsc')
    end
