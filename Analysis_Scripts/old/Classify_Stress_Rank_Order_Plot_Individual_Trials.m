...
% replace early HF values with NaNs
%%allCorrectTrialsByDip(3,:,:,1:32)=nan;
...
for iTrial=1:5
    ...
    for iLine=2:4
        ...
        line([tx,tx],[.1,1],'color','k','linewidth',4,'linestyle',':');
    end
    ...
    for iPlot=8:-1:1
        ...
        if ismember(iPlot,[1,2,4])
            
            hPlot(iPlot) = plot(smooth(squeeze(sum(allCorrectTrialsByDip(iPlot,:,iTrial,:),2)/size(allCorrectTrialsByDip,2)),10),...
                'linewidth',5,...
                'color',thisColor./255); hold on
            
        else
            
            hPlot(iPlot) = plot(smooth(1-squeeze(sum(allCorrectTrialsByDip(iPlot,:,iTrial,:),2)/size(allCorrectTrialsByDip,2)),10),...
                'linewidth',5,...
                'color',thisColor./255); hold on
        ...
        clear hResults
        hResults=squeeze(ttest(allCorrectTrialsByDip(iPlot,:,iTrial,:),mean(allCorrectTrialsByDipPerm(iPlot,:,iTrial,:,:),5)));
        
        % add lines for t-test results
        statLineYpos = statLineYpos+.015;
    ...
    saveas(h,[plotDir '/' sprintf('Classify_Stress_Rank_Order_Trial%02d.eps',iTrial)],'epsc')
