...
for iTOD = 1:2
    ...
    if iTOD==1
        idx = find(cortisol_CPT(:,10)==1);
        theseData = cortisol_CPT(idx,:);
        thisTitle = 'AM Start';
    else
        idx = find(cortisol_CPT(:,10)==2);
        theseData = cortisol_CPT(idx,:);
        thisTitle = 'PM Start';
    end
    ...
    % plot data
    errorbar(mean(theseData(:,7:9),1),std(theseData(:,7:9),0,1)./sqrt(size(theseData,1)),...
        'LineWidth',2.5,...
        'Color',[255,51,51]./255)
