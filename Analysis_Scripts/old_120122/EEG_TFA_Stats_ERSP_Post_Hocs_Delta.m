    ...
    for iExposures=1:5
        for iTimes=1
            for iCond=1:2
                ...
                %for iExposures=1:5
                if      iTimes==1; theseTimes = 2:26; % baseline   
...
%         %for iExposures=1:5
%         if      iTimes==1; theseTimes = 1:24; % baseline
%         elseif  iTimes==2; theseTimes = 65:94; % early immersion
%         elseif  iTimes==3; theseTimes = 95:124; % mid immersion
%         elseif  iTimes==4; theseTimes = 125:154; % late immersion
%         elseif  iTimes==5; theseTimes = 155:191; %    182:197;
...
%             postHocStats(iTimes).pValsObs = pValsObsParametric;
            ...
            allDeltaIntStats(iTimes).tStatIdx = tStatIdx_all;
            allDeltaIntStats(iTimes).tValsSigMat = sigMat;
            allDeltaIntStats(iTimes).tValsObs = tValsObs;
            %allPupilStats.ANOVA = allANOVA;
            allDeltaIntStats(iTimes).pValsObsParametric = pValsObsParametric;
            allDeltaIntStats(iTimes).nChans=length(statVecIdx);
