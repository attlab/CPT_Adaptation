%{
Physio_Plot_For_Manuscript_First_CPT_WPT_Only
Author: Tom Bullock
Date: 06.17.22
...
plotBlCorrectedPhysio = 0; % plotting absolute values here, so set to zero
...
if plotBlCorrectedPhysio==0
    timesForPlotting = 1:195; %TOM EDIT
else
...
%% create figure
h=figure('units','normalized','outerposition',[0 0 .6 1]); % 1 was .4
    
plotCnt = 0;
for iMeasure= [2,8,4,3,1,5,7,6]
...
%         elseif  iMeasure==10; allPhysio = all_RESP_PROCESSED; thisTitle1 = 'HF'; thisYlim = [-2,1]; thisYtick = [-2:.5:1];thisYlinePos = [5,4.7,4.4];thisLineWidth=7;thisLineGap = .1;
    ...
    % JUST ISOLATE FIRST TRIALS! [061722]
    allPhysio = squeeze(allPhysio(:,1,:,:));
    ...
    %Temporarily comment this chunk while I figure out the figure! [061722]
...
%     set(gca,'Visible','off','XLim',theseXlims,'yLim',[1,3])
    ...
    % plot CPT and WPT lines on the same subplot
    plotCnt = plotCnt + 1;
    subplot(4,2,plotCnt);
    for iCond=1:2
        ...
        thisMean = squeeze(mean(allPhysio(:,iCond,:),1));
        thisSEM = squeeze(std(allPhysio(:,iCond,:),0,1)/sqrt(size(allPhysio,1)));
        
        shadedErrorBar(linspace(1,xAxisLength,size(allPhysio,3)),thisMean,thisSEM,{'linestyle',thisLineStyle,'color','k'}); hold on
...
%         end
    ...
    % save image
    saveas(h,[destDir '/' 'Physio_First_CPT_WPT_Only.eps'],'epsc')
