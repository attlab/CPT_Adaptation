...
function EYE_Stats_Resample_Within_Deltas_ANOVA(baselineCorrect)
...
%% COMPUTE DELTA BETWEEN BL AND FINAL 15s OF EXPOSURE PERIOD
pa_mat_deltas = squeeze(nanmean(nanmean(paMatAll(:,:,:,:,141:155),5) - nanmean(paMatAll(:,:,:,:,26:40),5),4));

observedData = [squeeze(pa_mat_deltas(:,1,:)), squeeze(pa_mat_deltas(:,2,:))];
...
save([sourceDir '/' 'STATS_WITHIN_Resampled_EYE_n21_DELTAS.mat'],'allPupilStats','subjects','badSubs','pa_mat_deltas')
%else
