...
for iMeasure=[4,6] %1:8
    ...
    if useResampledStats==1 % get ANOVA results from resampled data mats
        ...
        for t=1:length(allCardioStats.ANOVA)
            condVec(t) = allCardioStats.ANOVA(t).var1.pValueANOVA; % base
            trialVec(t) = allCardioStats.ANOVA(t).var4.pValueANOVA; % base x cond
            intVec(t) = allCardioStats.ANOVA(t).var7.pValueANOVA; % base x cond x trial
        end
...
%         end
    ...
    if plotType==1
        if plotBlCorrectedPhysio
            saveas(h,[destDir '/' 'Physio_wStats_Bln_Within_' thisTitle1 '_anv_3way' '.eps'],'epsc')
        else
            saveas(h,[destDir '/' 'Physio_wStats_Raw_Within_' thisTitle1 '_anv_3way' '.eps'],'epsc')
        end
    end
